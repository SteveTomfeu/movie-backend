...
@app.get(
    ...
    description="Retourne l'évaluation d'un utilisateur pour un fil donné.",
...
def read_rating(
    ...
    if rating is None:
        raise HTTPException(
            status_code=404,
            detail=f"Aucune évaluation trouvée pour l'utilisateur {user_id} et le fim {movie_id}"
        )
    return rating

# Endpont pour obtenir une liste d'évaluations avec filtres
@app.get(
    "/ratings",
    summary="Lister les évaluations",
    description="Retourne une liste d'évaluations avec pagination et filtres ptionnels (film, utilisateur, note min).",
    response_description="Liste des évaluations",
    response_model=List[schemas.RatingSimple],
    tags=["évaluations"]
)
def list_ratings(
    skip: int = Query(0, ge=0, description="Nombres de résultats à ignorer"),
    limit: int = Query(100, le=1000, description="Nombre maximal de résultats à retourner"),
...
@app.get(
    ...
    description="Retourne un tag pour un utilisateur et un film doné, avec le texte du tag.",
    response_model=schemas.TagSimple,
    tags=["tags"]
)
def read_tag(
    user_id: int = Path(..., description="ID de l'utilisateur"),
    movie_id: int = Path(..., description="ID dufilm"),
